...
public class RoutingPacket extends Packet{
	...
	public boolean processFoundPacket(String port) {
		System.out.printf("Network %s is already in the routing table, port %s.\n", getDestNetwork().toString(), port);
		return false;
	}
	...
	public boolean processNotFoundPacket(String port) {
		System.out.printf("Adding network %s to the routing table, port %s\n", getDestNetwork().toString(), getPacketData());
		return true;
	}
}