...
public class RoutingTableEntry {
	...
	public int compareTo(IPAddress ip){
		return destinationip.compareTo(ip);
	}
	...
	public int compareTo(RoutingTableEntry entry){
		int ipCompare = this.destinationip.compareTo(entry.destinationip);
		if(ipCompare == 0){ // If these two ip addresses are equal, compare the two portcode Strings.
			return this.portCode.compareTo(entry.portCode);
		}
		else{
			return ipCompare;
		}
