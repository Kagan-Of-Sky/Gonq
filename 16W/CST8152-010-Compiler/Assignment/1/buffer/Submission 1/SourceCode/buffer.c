...
Buffer * b_create(short init_capacity, char inc_factor, char o_mode){
	...
	switch (o_mode){
		...
		case B_MODE_MULTIPLICATIVE_ARG:{
			if (unsigned_inc_factor == 0 || unsigned_inc_factor > 100){
				return NULL;
			}
			break;
		}
...
pBuffer b_addc(pBuffer const pBD, char symbol){
	...
	short newCapacity;      /* Used when expanding a multiplicative mode buffer. */
		...
		switch (pBD->mode){
			...
			case B_MODE_MULTIPLICATIVE: {
				...
				if ((newIncrement + pBD->capacity) > SHRT_MAX){
					...
					newCapacity = pBD->capacity + (short)newIncrement;          /* new capacity = current capacity + new increment */
				}
				
				if (newCapacity == pBD->capacity){ /* New capcity is the same as the current capacity, just max out the buffer size. */
					newBufferSize = SHRT_MAX;
				}
				else{
					newBufferSize = newCapacity;
				}
...
Buffer *b_pack(Buffer * const pBD){
	...
	newStorageBuffer = (char *)realloc(pBD->cb_head, pBD->addc_offset+1);
