...
static void   errorStoreExit(void);
...
Token aa_func02(char *lexeme){
	...
	/* Terminate the AVID prematurely if it is too long. */
	for (i=0; lexeme[i] && i<VID_LEN; ++i){
		/* No body. */
	}
	lexeme[i] = '\0';

	/* Check if the first letter specifies an integer type. */
	type = lexeme[0];
	if ('i' == type || 'o' == type || 'd' == type || 'n' == type){
		type = STVR_TYPE_INTEGER;
	}
	else{
		type = STVR_TYPE_FLOAT;
	}

	/* Insert this AVID into the symbol table. */
	i = st_install(sym_table, lexeme, type, line);

	if (i < 0){ errorStoreExit(); } /* Full symbol table. */

	t.attribute.vid_offset = i;
...
Token aa_func03(char *lexeme){
	...
	/* Move to last significant character in SVID. */
	for(i=0; i<VID_LEN-1 && lexeme[i]; ++i){
		/* No body */
	}
	
	/* Check if SVID is too long and need to truncate. */
	if('\0' != lexeme[i]){
		lexeme[i] = '#';
		++i;
		lexeme[i] = '\0';
	...
	/* Try to install lexeme. */
	i = st_install(sym_table, lexeme, STVR_TYPE_STRING, line);
 ...
 *                   In the future when more keywords are added to the language this search can greatly speed up performance. But not now.
 */
...
int countConsumeNewLine(Buffer *sc_buf, unsigned char c){
	switch (c){
		...
		/* Either windows or mac newline. */
		case '\r': {
			++line;
			if ('\n' != b_getc(sc_buf)){ /* Check if it is just a '\r' (mac), or a '\r' followed by a '\n' (windows). */
				b_retract(sc_buf);
			}
			return 2;
		}
...
/*
PURPOSE:          Print an error message, store the symbol table, and exits with EXIT_FAILURE.
AUTHOR:           Mark Kaganovsky
VERSION:          1.0
