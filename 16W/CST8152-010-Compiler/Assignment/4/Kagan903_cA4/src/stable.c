...
RETURN VALUE:     SUCCESS: A value >=0. This number can represent either an exisiting
                  ...
                  FAILURE: -1 The symbol table is full.
                           -2 The symbol table is invalid.
                           -3 Lexeme could not be copied into the character buffer.
                           -4 Invalid type argument.

ALGORITHM:        Uses st_lookup() to check whether the lexeme exists already.
*/
int st_install(STD sym_table, char *lexeme, char type, int line){
	STVR *newRecord; /* Pointer to new symbol table record. */
	int   lexLen;    /* Length of "char *lexeme". */
	int   i;         /* Generic int. */
...
RETURN VALUE:     SUCCESS: vid_offset.
                  FAILURE: -1 Type already updated.
                           -2 Invalid symbol table.
                           -3 Out of range vid_offset.
                           -4 Invalid v_type argument.
ALGORITHM:        none
*/
...
int st_update_value(STD sym_table, int vid_offset, InitialValue i_value){
	...
	memcpy(&stvr->i_value, &i_value, sizeof(i_value));
